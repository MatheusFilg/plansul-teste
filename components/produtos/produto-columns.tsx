...
export function getUniqueColumnValues<TData>(
  ...
  data.forEach((row) => {
    const path = columnId.includes('_') ? columnId.replace('_', '.') : columnId;
    
    let value = path.split('.').reduce((acc: any, key) => acc?.[key], row);
...
export const produtoColumns = (allTableData: Produto[]): ColumnDef<Produto>[] => [
  {
    accessorKey: "id",
    header: ({ column }) => {
